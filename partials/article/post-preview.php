...
<article class="post-preview  card-hover-primary-color">
    ...
    <div class="expertise-container">
        <?php $i = 0;
        foreach (get_the_terms($post->ID, 'expertises') as $expertise) :
            if ($i < 2) :
                $cat_color = get_field('color', $expertise);
                $category_link = get_term_link($expertise); ?>
                <span class="expertise-link"><a class="btn <?= $category_color; ?>-color" href="<?= $category_link; ?>"><?= $expertise->name; ?></a></span>
        <?php endif;
            $i++;
        endforeach; ?>
    </div>
